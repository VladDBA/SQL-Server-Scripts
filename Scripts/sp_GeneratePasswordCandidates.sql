/*
		Generate SQL Login password candidates
		Author: Vlad Drumea
		From: https://github.com/VladDBA/SQL-Server-Scripts/
		License: https://github.com/VladDBA/SQL-Server-Scripts/blob/main/LICENSE.md
		The procedure relies on STRING_SPLIT so it only works on SQL Server 2016 and newer
		More info: 
*/
SET ANSI_NULLS ON;
...
CREATE OR ALTER PROCEDURE dbo.sp_GeneratePasswordCandidates
(
       @BaseWordsList             NVARCHAR(1200) = N'', /*Comma-separated list of custom base words*/
       @UseInstInfo               BIT = 1               /*Set to 1 to use database names, logins, and instance name for password candidates*/
)
AS
SET ANSI_PADDING ON;
...
BEGIN
       ...
       RAISERROR('Generating password candidates in the #WordList table', 0, 1) WITH NOWAIT;
       ...
       IF @BaseWordsList = N'' OR @BaseWordsList IS NULL
              SET @BaseWordsList = @Common;
       ELSE
              SET @BaseWordsList = @BaseWordsList + N',' + @Common;

       /*Cleanup any potential spaces after or before the comma*/
       SELECT @BaseWordsList = REPLACE(REPLACE(@BaseWordsList, N', ', N','), N' ,', N',');
